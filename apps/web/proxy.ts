...
export function proxy(req: NextRequest) {
  ...
  if (pathname.startsWith("/api/auth/sign-up")) {
    // Sign-up is a public Better Auth endpoint (only in the matcher so we can
    // block it); pass it through unless registration is disabled.
    return registrationDisabled
      ? NextResponse.json({ error: "registration_disabled" }, { status: 403 })
      : NextResponse.next();
  }
  if (registrationDisabled && pathname === "/register") {
