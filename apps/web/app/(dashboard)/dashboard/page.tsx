...
export default async function DashboardPage() {
  ...
  const [[{ totalSessions }], [{ totalCompetitions }], pbs, recent] = await Promise.all([
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-5">
            <p className="text-sm text-muted-foreground">PB di stagione</p>
            <p className="mt-1 text-2xl font-semibold tabular-nums">{seasonBests.length}</p>
          </CardContent>
        </Card>
      </div>
